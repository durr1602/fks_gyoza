...
df["C_both_sides"] = df.sequence.apply(
    lambda x: True if (x[0] == "C") & (x[-1] == "C") else False
)
...
gby = df_filtered.groupby("sequence")[["header", "RK_count"]].agg(
    n_homologues=("header", "nunique"),
