...
def dataframe_to_multifasta(df, output_file):
    ...
    for idx, row in df.iterrows():
        ...
        seq = Seq(sequence)
        record = SeqRecord(seq, id=f"{numerical_id}", 
...
def position_to_df(alignment_file):
    ...
    df_positions = pd.DataFrame.from_dict({(name,alignment_index): positions[name][alignment_index]
                                           for name in positions.keys()
                                           for alignment_index in positions[name].keys()},
                                          orient='index', columns=['aa', 'Ungapped position'])

    # Resetting index and splitting 'info' column into separate columns
    df_positions = df_positions.reset_index(names=['info'])
    df_positions[['info', 'Alignment position']] = df_positions['info'].apply(lambda x: pd.Series([x[0], x[1]]))
    df_positions[['ID', 'ortho_homolog', 'species', 'uniprot', 'mutation']] = df_positions['info'].str.split('|', expand=True)

    # Extracting values after '=' in each column and filling missing values
    df_positions['ortho_homolog'] = df_positions['ortho_homolog'].str.split('=').str[1].str.strip().fillna('NA')
    df_positions['species'] = df_positions['species'].str.split('=').str[1].str.strip().fillna('NA')
    df_positions['uniprot'] = df_positions['uniprot'].str.split('=').str[1].str.strip().fillna('NA')
    df_positions['mutation'] = df_positions['mutation'].str.split('=').str[1].str.strip().fillna('NA')
...
def get_wild_type_hotspots(row, df):
    ...
    '''
    wild_type_row = df[(df['species'] == row['species']) & (df['mutation'] == 'Wild-type') &
                       (df['ortho_homolog'] == row['ortho_homolog']) & (df['Hotspot'] == row['Hotspot'])]
...
fks_mut = fks[['species','gene or protein name','ortho_homolog','mutation','mutation_type']].drop_duplicates()

# Split mutation in wt AA, position and alt AA
fks_mut[['wt_AA', 'position', 'alt_AA']] = fks_mut['mutation'].str.extract(r'(\D+)(\d+)(\D+)')

# Fix insertion cases: extract first number as position, AA after "ins" as alt_AA
ins_mask = fks_mut['mutation'].str.contains('ins')
fks_mut.loc[ins_mask, 'position'] = fks_mut.loc[ins_mask, 'mutation'].str.extract(r'(\d+)')[0]
fks_mut.loc[ins_mask, 'alt_AA'] = fks_mut.loc[ins_mask, 'mutation'].str.extract(r'(ins[A-Za-z]+)')[0]


# %% Mutate FKS sequences with mutation from fungAMR database
fks_mut = pd.merge(fks_mut, sequences, on=['species','ortho_homolog', 'gene or protein name'], how='left')
fks_mut = fks_mut.dropna(subset = ['sequence'])
fks_mut['mutated_seq'] = fks_mut.apply(lambda row: mutate_sequence(row['sequence'], row['position'], row['alt_AA'], row['mutation_type']), axis=1)

# Add wild-type orthologues sequences
fks_wt = sequences[(sequences['gene or protein name'] == 'Fks1') | (sequences['gene or protein name'] == 'Fks2')].drop_duplicates()
...
def extract_hotspot_sequences(df, species, ortho, start_pos, end_pos, hotspot_label, ortho_label):
    # Get alignment positions for start and end
    start = df[(df['species'] == species) &
               (df['mutation'] == 'Wild-type') &
               (df['ortho_homolog'] == 'Fks') &
               (df['Ungapped position'] == start_pos)]['Alignment position'].values[0]

    end = df[(df['species'] == species) &
             (df['mutation'] == 'Wild-type') &
             (df['ortho_homolog'] == 'Fks') &
             (df['Ungapped position'] == end_pos)]['Alignment position'].values[0]

    # Filter and aggregate
    hotspot_df = df[df['Alignment position'].between(start, end)].drop_duplicates()
    hotspot_df = hotspot_df.groupby(['species', 'ortho_homolog', 'uniprot', 'mutation']).agg({
        'aa': ''.join,
        'Ungapped position': lambda x: x.tolist()
    }).reset_index()
...
# Fks1
Fks1_HS1_mutated_seq = extract_hotspot_sequences(fks1_mutated_positions, 'Saccharomyces cerevisiae', 'Fks', Sc_fks1_hs1_start, Sc_fks1_hs1_end, 'HS1', 'Fks1')
Fks1_HS2_mutated_seq = extract_hotspot_sequences(fks1_mutated_positions, 'Saccharomyces cerevisiae', 'Fks', Sc_fks1_hs2_start, Sc_fks1_hs2_end, 'HS2', 'Fks1')
Fks1_HS3_mutated_seq = extract_hotspot_sequences(fks1_mutated_positions, 'Saccharomyces cerevisiae', 'Fks', Sc_fks1_hs3_start, Sc_fks1_hs3_end, 'HS3', 'Fks1')

# Fks2
Fks2_HS1_mutated_seq = extract_hotspot_sequences(fks2_mutated_positions, 'Saccharomyces cerevisiae', 'Fks', Sc_fks2_hs1_start, Sc_fks2_hs1_end, 'HS1', 'Fks2')
Fks2_HS2_mutated_seq = extract_hotspot_sequences(fks2_mutated_positions, 'Saccharomyces cerevisiae', 'Fks', Sc_fks2_hs2_start, Sc_fks2_hs2_end, 'HS2', 'Fks2')
Fks2_HS3_mutated_seq = extract_hotspot_sequences(fks2_mutated_positions, 'Saccharomyces cerevisiae', 'Fks', Sc_fks2_hs3_start, Sc_fks2_hs3_end, 'HS3', 'Fks2')

# Combine all mutated seq
Fks1_mutated_seq = pd.concat([Fks1_HS1_mutated_seq, Fks1_HS2_mutated_seq, Fks1_HS3_mutated_seq])
Fks2_mutated_seq = pd.concat([Fks2_HS1_mutated_seq, Fks2_HS2_mutated_seq, Fks2_HS3_mutated_seq])
...
Fks_mutated_seq['wild_type_hotspot']= Fks_mutated_seq.apply(lambda row: get_wild_type_hotspots(row, Fks_mutated_seq), axis=1)

# Filter out rows where both hotspot sequences are the same as the wild-type hotspot sequences
Fks_mutated_seq_filtered = Fks_mutated_seq[Fks_mutated_seq['aa'] != Fks_mutated_seq['wild_type_hotspot']]
...
fks_metadata = fks[['species','gene or protein name','drug','mutation',
                    'pubmedid','confidence score','ortho_mut', 'MIC',
                    'Strongest resistance evidence reported',
                    'Strongest sensitivity evidence reported']]

fks_mutated_final = pd.merge(Fks_mutated_seq_filtered, fks_metadata, left_on=['species','ortho_homolog','mutation'],
                             right_on=['species','gene or protein name','mutation'], how = 'left')

# %% Convert column "mutation" to Scer numbering
alignment_positions = pd.read_csv(f'{wkdir}/master_positions_df_030425.csv', index_col=0).drop(columns={'uniprot'})

fks_mutated_final['ortho_homolog'] = "Fks"
fks_mutated_final[['wt_AA', 'position', 'alt_AA']] = fks_mutated_final['mutation'].str.extract(r'(\D+)(\d+)(\D+)')

fks_mutated_final['position'] = pd.to_numeric(fks_mutated_final['position'])
fks_mutated_final_aligned = pd.merge(fks_mutated_final, alignment_positions, on=['species', 'ortho_homolog', 'gene or protein name', 'position', 'wt_AA'], how='left')

fks_mutated_final_aligned['conversion_species'] = "Saccharomyces cerevisiae"

fks_mutated_final_aligned['convert_pos'] = fks_mutated_final_aligned.apply(update_position, axis=1, alignment_positions=alignment_positions)
fks_mutated_final_aligned['convert_wt'] = fks_mutated_final_aligned.apply(update_wt, axis=1, alignment_positions=alignment_positions)

fks_mutated_final_aligned['Scer_mutation'] = fks_mutated_final_aligned['convert_wt'] + fks_mutated_final_aligned['convert_pos'].astype('Int64').astype(str) + fks_mutated_final_aligned['alt_AA']
fks_mutated_final_aligned.drop(columns=['wt_AA', 'position', 'alt_AA','conversion_species','convert_pos','convert_wt','alignment_pos','accession'], inplace=True)
